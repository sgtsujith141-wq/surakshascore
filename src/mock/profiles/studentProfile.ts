...
/**
 * Realistic profile representing a typical university student:
 * Strong device updates, screen lock enabled, but weak password management and occasional link risk.
 */
export function createStudentProfile(timestamp: string = new Date().toISOString()): SignalMap {
  const signals: Record<string, Signal> = {
    'account.two_factor_primary_email': {
      ...
      status: 'AVAILABLE',
      value: 'sms_otp', // Secure (has 2FA, albeit SMS)
      timestamp,
    ...
    'account.password_reuse': {
      ...
      tier: EvidenceTier.TIER_4_SELF_REPORTED,
      status: 'AVAILABLE',
      value: 'frequent', // Critical finding: password reuse
      timestamp,
      source: 'assessment_survey',
    },
    ...
    'phishing.opens_unverified_links': {
      ...
      tier: EvidenceTier.TIER_4_SELF_REPORTED,
      status: 'AVAILABLE',
      value: true, // High finding: clicks unknown links
      timestamp,
      source: 'assessment_survey',
    },
    ...
    'privacy.unreviewed_broad_permissions': {
      ...
      tier: EvidenceTier.TIER_2_OS_API,
      status: 'AVAILABLE',
      value: true, // High finding: unreviewed permissions
      timestamp,
